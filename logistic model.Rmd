...
main_effects_model <- glm(formula_main_effects, data = balance_data_dummies, family = binomial())
summary(main_effects_model)

# Add interaction terms incrementally
interaction_terms <- c("age:grade", "tumor_size:estrogen_status", 
                       "progesterone_status:regional_node_examined", 
                       "marital_status_Single:reginol_node_positive")

for (term in interaction_terms) {
  formula_with_term <- as.formula(paste(deparse(formula_main_effects), "+", term))
  model <- glm(formula_with_term, data = balance_data_dummies, family = binomial())
  cat("\nAdding term:", term, "\n")
  print(summary(model))
}
```
