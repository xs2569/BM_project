...
confusionMatrix(as.factor(balance_data$predicted_classes_aic), as.factor(balance_data$Status))


predictions_bic <- predict(logit_bic, type = "response")
balance_data$predicted_classes_bic <- ifelse(predictions_bic > 0.5, 1, 0)
confusionMatrix(as.factor(balance_data$predicted_classes_bic), as.factor(balance_data$Status))

##AUC-ROC:
library(pROC)
roc_curve_aic <- roc(balance_data$Status, predictions_aic)
roc_curve_bic <- roc(balance_data$Status, predictions_bic)
plot(roc_curve_aic, col = "blue", main = "ROC Curve for AIC and BIC Models")
lines(roc_curve_bic, col = "red")
auc(roc_curve_aic)
auc(roc_curve_bic)
```
