...
balance_data$status <- as.factor(balance_data$status)
balance_data$race <- as.factor(balance_data$race)
balance_data$marital_status <- as.factor(balance_data$marital_status)
balance_data$x6th_stage <- as.factor(balance_data$x6th_stage)
balance_data$a_stage <- as.factor(balance_data$a_stage)


# Generate all two-way interactions
formula_with_interactions <- status ~ (age + marital_status + grade +  tumor_size + estrogen_status + progesterone_status + regional_node_examined + reginol_node_positive)^2 

# Fit the full model with all two-way interactions
full_model <- glm(formula_with_interactions, data = balance_data, family = binomial())


# Perform Stepwise Selection (AIC)
aic_forward_model <- stepAIC(full_model, direction = "forward", trace = FALSE)
aic_backward_model <- stepAIC(full_model, direction = "backward", trace = FALSE)
aic_both_model <- stepAIC(full_model, direction = "both", trace = FALSE)

# Perform Stepwise Selection (BIC)
bic_forward_model <- stepAIC(full_model, direction = "forward", trace = FALSE, k = log(nrow(balance_data)))
bic_backward_model <- stepAIC(full_model, direction = "backward", trace = FALSE, k = log(nrow(balance_data)))
bic_both_model <- stepAIC(full_model, direction = "both", trace = FALSE, k = log(nrow(balance_data)))
```
...
```
```{r}
# Extract AIC Model Coefficients
aic_summary <- summary(aic_model)
aic_coef <- as.data.frame(aic_summary$coefficients)
aic_coef$Term <- rownames(aic_coef)  # Add variable names
aic_coef$Model <- "AIC"  # Add model identifier
colnames(aic_coef) <- c("AIC_Estimate", "AIC_Std.Error", "AIC_z.Value", "AIC_P.Value", "Term", "Model")

# Extract BIC Model Coefficients
bic_summary <- summary(bic_model)
bic_coef <- as.data.frame(bic_summary$coefficients)
bic_coef$Term <- rownames(bic_coef)
bic_coef$Model <- "BIC"
colnames(bic_coef) <- c("BIC_Estimate", "BIC_Std.Error", "BIC_z.Value", "BIC_P.Value", "Term", "Model")

# Merge AIC and BIC Results by Term
comparison_table <- merge(
  aic_coef[, c("Term", "AIC_Estimate", "AIC_Std.Error", "AIC_z.Value", "AIC_P.Value")],
  bic_coef[, c("Term", "BIC_Estimate", "BIC_Std.Error", "BIC_z.Value", "BIC_P.Value")],
  by = "Term", 
  all = TRUE
)
...
comparison_table <- comparison_table[, c("Term", 
                                         "AIC_Estimate", "AIC_Std.Error", "AIC_z.Value", "AIC_P.Value",
                                         "BIC_Estimate", "BIC_Std.Error", "BIC_z.Value", "BIC_P.Value")]
...
# Export the comparison table to an Excel file
#write_xlsx(comparison_table, "aic_bic_comparison.xlsx")

# Confirm Export
cat("The comparison table has been successfully written to 'aic_bic_comparison.xlsx'")
...
# Log-Likelihood for AIC and BIC Models
logLik_aic <- logLik(aic_model)
logLik_bic <- logLik(bic_model)

cat("Log-Likelihood for AIC Model:", logLik_aic, "\n")
cat("Log-Likelihood for BIC Model:", logLik_bic, "\n")

# Predictions and Classification for AIC Model
predictions_aic <- predict(aic_model, type = "response")
balance_data$predicted_classes_aic <- ifelse(predictions_aic > 0.5, 1, 0)

# Predictions and Classification for BIC Model
predictions_bic <- predict(bic_model, type = "response")
balance_data$predicted_classes_bic <- ifelse(predictions_bic > 0.5, 1, 0)
...
# Confusion Matrices
library(caret)
conf_matrix_aic <- confusionMatrix(balance_data$predicted_classes_aic, balance_data$status)
conf_matrix_bic <- confusionMatrix(balance_data$predicted_classes_bic, balance_data$status)

cat("Confusion Matrix for AIC Model:\n")
print(conf_matrix_aic)

cat("Confusion Matrix for BIC Model:\n")
...
# AUC for AIC and BIC Models
auc_aic <- auc(roc_curve_aic)
auc_bic <- auc(roc_curve_bic)

cat("AUC for AIC Model:", auc_aic, "\n")
cat("AUC for BIC Model:", auc_bic, "\n")
