...
##project_2_data <- project_2_data |>
##  dpyler::select(-c(t_stage, n_stage, reginol_node_positive, differentiate))
...
cox_model <- coxph(Surv(survival_months, status) ~ .-a_stage, data = balance_data)
vifs <- vif(cox_model)
print(vifs)
...
Bic_model <- coxph(Surv(survival_months, status) ~ age + x6th_stage + grade + 
